...
const MAX_VISIBLE_PAGES = 3 // Number of page numbers to display in the pagination bar
    ...
    return (
      <aside className="flex mt-10">
        <ul className="ml-auto flex gap-x-3 items-center">
          <li onClick={() => setCurrentPage(currentPage => currentPage === 1 ? currentPage : Math.max(1, currentPage - 1))} className="inline-flex list-none rounded-md h-9 w-10 bg-primary-100 cursor-pointer">
          ...
          <li onClick={() => setCurrentPage(currentPage => currentPage === endPage ? currentPage : Math.min(totalPages, currentPage + 1) )} className="inline-flex cursor-pointer list-none rounded-md h-9 w-10 bg-primary-100">
            <button className="mx-auto">
              <svg className="w-5 h-5" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
